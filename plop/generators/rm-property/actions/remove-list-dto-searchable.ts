...
export default function removeListDtoSearchable(_plop: NodePlopAPI): ActionType {
  return {
    type: 'removeBlock',
    data: {
      path: 'src/modules/{{kebabCase moduleName}}/dto/list-{{kebabCase entityName}}.dto.ts',
      tag: 'searchable-field',
      name: '{{camelCase name}}',
    },
    skip: (answers: Record<string, any>) => {
      if (!answers.confirm) return 'remove-list-dto-searchable - aborted by user';
      if (!listDtoExists(answers)) return 'remove-list-dto-searchable - list-dto missing';
    },
  };
}
