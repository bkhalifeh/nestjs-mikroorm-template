...
export default function removeServiceSearch(_plop: NodePlopAPI): ActionType {
  return {
    type: 'removeBlock',
    data: {
    ...
    skip: (answers: Record<string, any>) => {
      if (!answers.confirm) return 'remove-service-search - aborted by user';
      if (!serviceExists(answers)) return 'remove-service-search - service missing';
    },
  };
}
